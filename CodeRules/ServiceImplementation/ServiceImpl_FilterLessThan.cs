    ...
    public class ServiceImpl_FilterLessThan : ServiceImplExtensionRule
    {
        ...
        public override bool? Verify(ServiceContext context, out ExtensionRuleViolationInfo info)
        {
            ...
            for (int n = 0; n < entities.Count; n++)
            {
                ...
                {
                    bool test = false;
                    foreach (var t  in filterRestrictions.Item2)
                    {
                        test = entities[n][t.PropertyName].HasValues;
                        if(test)
                        {
                            primitivePropName = t.PropertyName;
                            break;
                        }
                    }
                    if (!test)
                    ...
                    propVal = entities[n].Value<Int64>(primitivePropName) + 1;
                    break;
                }
                catch 
                {
